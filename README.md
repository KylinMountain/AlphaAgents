...
**这条门槛与 `n<50 不上线` 并不一致，而代码站在前者那边。** 晋升重检读的是
**冻结版本自己声明的**门槛（今天 = 20），所以 n 落在 20–49 的裁决既过闸门、也过版本，
却违反 `docs/GOLDEN_PRINCIPLES.md` §7，没有任何东西拦它。这条差异现在由
`scripts/policy.py status` 每次打印（`holdout_gate.promotion_floor_gap`）；
修不修是操作者的决定——把常数提到 50 会让**所有已冻结版本立刻变 drifted**，
因为冻结之后的行为性改动本来就该是一个新候选。
