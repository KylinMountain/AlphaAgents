...
所以「还没有评估货币」是一句关于日历的话，不是关于代码的话 —— 但**日历要按交易日算**：
到期判定用的是 `date + 5 天`（日历天），而 `_forward_return` 要 6 个**交易日**的收盘，
于是每一笔预测在它「到期」那天都必然拿不到数据（记在 tech-debt D10）。
最早那批（09-08）要等 **2026-09-15** 的收盘进库才可评。
