...
**The chain.** Design §4: ``thesis_id → order_id → fill_id →
ledger_entry_id``. Every hop is an explicit column here. The alternative —
finding the nearest live thesis with the same stock code — is not an
ownership relation, and with two traders on one stock it hands one book's
result to the other's idea.
