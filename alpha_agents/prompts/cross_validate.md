...
1. 资金面 — 调用 get_stock_fund_flow 查主力资金方向，调用 get_margin_data 查融资融券余额变化，调用 get_lhb_detail 查龙虎榜机构动向
   - **注意**：个股级北向持仓数据自 2024-08-19 起不可用，不要验证"该股北向是否增持"。get_north_flow 只能查汇总（大盘北向净流入/流出），不能查单只股票
   - 通过：主力连续流入 或 融资余额持续增加 或 龙虎榜机构净买入
   - 不通过：主力连续流出 且 融资余额下降 且 龙虎榜机构净卖出
