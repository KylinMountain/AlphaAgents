...
1. 资金面 — 调用 get_stock_fund_flow 查主力资金方向，调用 get_north_flow 查北向持仓，调用 get_margin_data 查融资融券余额变化，调用 get_lhb_detail 查龙虎榜机构动向
   - 通过：主力连续流入 或 北向增持 或 融资余额持续增加 或 龙虎榜机构净买入
   - 不通过：主力连续流出 且 北向减持 且 融资余额下降 且 龙虎榜机构净卖出
