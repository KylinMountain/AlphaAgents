...
**The repository's floor is `n ≥ 20` paired samples, and it is a minimum governance condition, not sufficient evidence of improvement.** It is declared in [golden principles](GOLDEN_PRINCIPLES.md) §7 and enforced by the promotion gate: `holdout_gate.MIN_VALIDATION_SAMPLES = 20` refuses a verdict below it, and `policy_registry` re-checks the same number out of the *frozen version's* own `rules` block, so a version cannot loosen its own floor. The bar was 50 until 2026-09-15, when three documents declaring 50 while the code enforced 20 was closed by **lowering the rule to meet the code**; the other direction — raising the constant — would have marked every version already frozen as drifted. `holdout_gate.GOVERNANCE_MIN_SAMPLES` quotes the rule so a *version* declaring less can still be reported, and `scripts/policy.py status` prints the version's floor, the gate's abstention line, and the rule. Twenty is a weaker basis than fifty; that cost is accepted and named rather than hidden.
...
| 4 — controlled evolution ✅ delivered 2026-09-13 | Frozen policy registry, forward shadow accounts, fair evaluation, authorized promotion, and rollback | Evidence controls future policy changes through one audited entry point. **Extended 2026-09-13: the entry point can create its first record (`freeze` / `install`), the decision parameters it puts in force are read from the version in force, and the build registers one candidate producer — so the loop is runnable. Nothing has been promoted in production, and no shadow run has been opened there; see §14 of the implementation status.** |
