...
**The repository's `n ≥ 50` requirement is a minimum governance condition, not sufficient evidence of improvement or 50 independent observations.** It is declared in [golden principles](GOLDEN_PRINCIPLES.md) §7 and is **not enforced by any code**: the promotion path re-checks `n` against the floor the *frozen version* declares (`holdout_gate.MIN_VALIDATION_SAMPLES`, 20 at the time of writing), so a verdict between 20 and 49 satisfies the gate and this rule at once and nothing refuses it. `holdout_gate.GOVERNANCE_MIN_SAMPLES` quotes the number so the two can be compared rather than conflated, `holdout_gate.promotion_floor_gap` names the difference, and `scripts/policy.py status` prints it. The gap is an open decision — closing it means raising the constant and drifting every version already frozen — not a measured achievement.
