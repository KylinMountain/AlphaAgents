...
- **回填语料有一个结构性缺口，用它之前必须知道（2026-09-16 保真校验）。**
  比对实盘窗口（09-07 → 09-15）：实盘 3,595 条、回填 3,876 条、完全相同 3,299 条，
  **只在实盘里有 291 条（按标题占 8.1%）**。这 291 条**全是付费栏目** ——
  `【研选•研报数据】`/`【风口研报】`/`【盘中宝】`/`【大佬持仓跟踪】`/`【机构调研】`/
  `【九点特供】`/`【点金互动易】`/`【玩转ETF】`。
  实测机制：`last_time = now` 返回 4 条 `【】`栏目，而 **−2 小时 / −3 天 / −10 天全为 0**
  （`rn=30` 也为 0，**排除分页大小**）⇒ **CLS 只把付费栏目发给贴近当下的游标**。
  所以重建语料在**全部历史**上都只有免费快讯流，而实盘的 `news_items` 是两者兼有。
  **任何基于回填语料的入场结论都只能声称「快讯流的效果」，不能声称「整套新闻输入的效果」。**
  反向的好消息：回填在免费快讯流上**是超集**（+568 条实盘轮询因翻页边界丢掉的）。
  补法（未做）：另寻 CLS 栏目接口，同一套 sign 手法值得一试。
- **回填已完成（2026-09-16 09:26）。** `done: 18712 pages, 935600 rows, cursor 2020-01-01`，
  13h10m；库里 `财联社电报(历史回填)` **935,850 条**，覆盖 **2019-12-31 → 2026-09-15**；
  非规范时间戳/重复 hash/空标题**均为 0**，`quick_check: ok`。全库新闻 982,533 条 / 1,057 来源，
  库体积 1.2 GB。密度逐年上升：2020 **276**/日 → 2026 **486**/日。
  归档其实更深（`2016-01-03` 有数据、`2014-01-02` 为 0），但**两个价格库都从 2020-01-02 起**
  ⇒ 更早的新闻没有可对分的价格，**下限设 2020-01-01 是正确的**。
- **接新闻之前要定的那个参数，现在有数据了：按板块名匹配覆盖 44.1%。**
  用 1,105 个板块名（概念+行业）做字面匹配，抽 25 个交易日 / 10,781 条新闻：
  **44.1% 至少命中一个板块名**，逐日 32%–51%。**方向要标注**：2 字名会在无关文本里命中，
  「提到」≠「相关」⇒ 44% 是**召回上界、精度未知**。
  于是三个候选（**需人定**）：
  1. **名称提及计数**当第三个信号并进 cross-section —— 最便宜、无 LLM、覆盖 44%；
  2. **LLM 主题抽取** —— 更接近实盘，但多一层「不可复现」（重跑是新判断，不是当初那次）；
  3. **只当过滤器**：只保留当天被提及的板块，看门的分离是否**变好** ——
     这一条最直接回答 D6「新闻那一层到底有没有信号」，建议先做它。
