...
六件事，全部是「让历史证据可信」的前置条件。**进度：3/6**（2026-09-16 当天）。

- [ ] **语料只读** —— `market_history.db` 与新闻库以只读方式打开，回放期间不得写入。
      **未做**：`walk_bootstrap.py` 只是**按引用共享**并把它打印出来，**没有任何东西强制只读**。
...
- [ ] **第一版只有开盘执行**，不发明日内路径 —— 见 §4。**未做**：需要一个成交模型
      （开盘价 + `intraday_ambiguous`），而它属于 M1。
