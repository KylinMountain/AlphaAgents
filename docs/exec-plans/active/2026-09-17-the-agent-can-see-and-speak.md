...
状态：completed（四阶段落地、8 个缺陷修复、端到端复跑验证通过）
创建：2026-09-17
...
| 等权大盘 | **-2.807%** |
| 超额 | **+2.275%** |
| 出场归因 | 机械 6 笔（止损 5、止盈 1） |
| agent 决策 | `agent_exit_calls 12`、`agent_trim 3`、T+1 拦下 7 |

**注：每一轮的绝对收益都不同，这正是本仓库记录在案的事实**——回放是单次
采样，模型权重已读过这些日期，所以两次运行的数字不可比，也不构成策略证据。
可比的是**机制**：面板字段、决策路径、错误数、计数。
