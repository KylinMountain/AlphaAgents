...
- **没有估值/财务数据**，本轮不含。
- **资金流没有接进面板，这是目标里点名的一项，如实记为未交付。**
  `stock_fund_flow_snapshots` 只有 **252 行 / 47 个代码**（占 5216 只的
  **0.9%**），而且写入函数 `save_stock_fund_flow(code, ...)` 是**按需单只**
  缓存——某个调用问过哪只就存哪只，不是每日全量快照。给它加一列会让
  **约 99% 的行是空白**，是噪音不是信息。要真正交付这一项，需要先有一个
  每日全量拉取的资金流入库任务。
...
| agent 决策 | `agent_exit_calls 8`、`agent_trim 2`、T+1 拦下 9 |
