...
### 追加验收（2026-09-26，复核最终回放后）

> 2026-09-26 稍后：下文的 `HARD_STOP_PCT` 硬线已被
> [没有系统出场线](2026-09-26-no-system-exit-lines.md) 取代——`--live-exits` 现在只执行
> agent 自设的止损/止盈价，系统不再有任何出场线。以下保留为当时的记录。

最终 30 日回放的 335 个 Runtime 决策全部是 BUY / WAIT / REJECT，31 笔卖出全部是
入场时写下的止损/止盈价被机械触发（报告：机械 31、agent 0）。原因：验收命令未传
`--agent-exits`，回放默认由价格线平仓；而生产 `AGENT_EXIT_DECISIONS=1`，卖出由 agent
决定、只保留 `HARD_STOP_PCT` 硬线。回放与实盘的出场策略不同，违反 T8 "replay 不再拥有
另一套交易策略"。因此上面的 30 日结果只证明买入侧与管道，不证明 T5 持仓管理。

- [x] `--live-exits`：agent 决定卖出；不执行入场止损价与止盈价；仅在成本价亏损达
  `HARD_STOP_PCT` 时硬平仓（跳空穿线以开盘价成交）。与 `--autonomous`、实验 manifest
  互斥。单元测试钉住三种价格情形。
- [x] 报告的"出场"说明按实际模式输出；硬线平仓单独归类为「硬止损」，不与 agent 或入场
  止损混计。
- [x] 3 个交易日的真实模型 `--live-exits` 回放：持仓日 Runtime 决策中 HOLD/REDUCE/SELL
  数 > 0，且所有非硬线卖出的 fill 都以 `agent` 开头。
  实测（2026-09-26，`5d85dd2`，run `live3-20260105`，`2026-01-05 → 01-07`，退出 0）：
  34 个 Runtime 决策中 HOLD 7、REDUCE 1（此前 30 日为 0）；唯一卖出为
  `agent减仓 605333`，理由写明"未及 -8% 硬线"；机械出场 0；HOLD 均带 reasoning、
  `made_at 09:35`、`evidence_scope=replay_daily` 封存进 TraderState。无备用模型调用，
  生产库 hash 未变，语料只读。首日入场未成交，故只有 1 月 7 日一天持仓可卖；它证明
  持仓管理接通，不构成出场质量或收益结论。

## 决策记录

- 2026-09-26：新增 `--live-exits` 而不是改默认值。默认值承载既有窗口的可比性和实验
  manifest 的 runtime 契约；把回放对齐实盘是一个显式、写进报告的模式。`--autonomous`
  保留为"无安全网"实验臂，它也不是实盘口径。
