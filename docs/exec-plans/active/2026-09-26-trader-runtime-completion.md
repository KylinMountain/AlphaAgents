...
- [x] C4：两支分支各 ≥3 个交易日退出 0；注入分支中至少一条决策的上下文含该 Rule，
  且报告两支的动作差异数（可以为 0——为 0 是结论，不是失败）。
  实测（2026-09-26，检查点 `t9-prefix` 2026-02-24→02-25，分支 02-26→03-04 各 5 日，1 trial）：
  Rule =「5 日累计涨幅 ≥10% 时买入：判错 21/35」（由 1/5–2/13 窗口挖出，前向检验）。

  | | control | rule_run_up_5d |
  |---|---|---|
  | 含 Rule 的决策上下文 | — | 5 |
  | buy / 其中 run_up_5d | 9 / 8 | 7 / 6 |
  | hold / reduce / sell | 22 / 4 / 6 | 19 / 8 / 7 |
  | 5 日收益 | −0.78% | −0.59% |

  **结论：Rule 每天都到达了决策上下文，但行为几乎没变**——受 Rule 约束的情形仍占买入的
  6/7（对照 8/9）。1 个 trial、独立模型轨迹，差异在采样噪声内。以提示词文字注入的 Rule
  不足以改变交易员行为；这是下一个设计问题，不是本项的失败。
  两支被报告为 incomplete，仅因 `production_db_unchanged=否`：本机实盘进程 `run-v2`
  在同时写生产库；逐表检查生产库中无任何 `fork-*`/`t9*` 行，隔离成立。
- [x] C6 卖出侧学习：`exit_decision` 的实盘与回放入口都注入 horizon=`position` 的
  Lesson/Rule（此前只有买入侧注入，持有/卖出格子学到也到不了卖出决策）。测试：position
  Rule 进入卖出上下文；3-5d 的买入 Rule 与其他 run 的 Rule 不进入。
