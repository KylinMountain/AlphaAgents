...
## 两轮 20 日回放（2026-01-05 … 01-30），两个半边

一轮机械出场、一轮 `--agent-exits`，合起来覆盖 Trade / Learn / Evolve。
两轮 `exit=0`、抛错 0、空内容 0，口径检查均为「生产库内容 hash 未变 /
语料只读 / 模型调用与声明一致」。

| | 机械出场 | `--agent-exits` |
|---|---|---|
| 买入 / 卖出 | 8 / 7 | 2 / 4 |
| 出场归因 | 机械 7（止损 6、止盈 1）、agent **0** | 机械 1（止损）、agent **3**（减仓 2、清仓 1）|
| 平仓打标 | 7 笔 | 2 笔 |
| 写下的观察 | **4 条** | 0 条（平仓数不足以成候选）|
| LLM 调用 / 工具调用 | 162 / 391 | 168 / 390 |

### Trade：买得像交易员

`603712 七一二` 的下单理由（已进日志，不再只在 journal 里）：

> 它是候选里结构最干净的低吸对象……七一二自身 20 日仅 +0.71%（没透支），
> T-1 +5.27% 却只有 4.81% 换手，**是靠低换手推进而不是筹码堆出来的急拉**，
> 且无连板、近 20 日无涨停，不存在「已经封板抢不到」的问题。

### Trade：也卖得像交易员

`--agent-exits` 那轮，`000988 华工科技`：

> 买入逻辑「存储芯片为当前最强主线」**已证伪**：该线累计强度退至 3/10、
> 今日仅 +1，**资金覆盖从 188/199 的最强档明显收缩**，属于催化（三星利润
> +涨价）落地后的资金退潮。个股峰值浮盈 +3.4% 已回吐 73% 至 +0.91%……
> 且 **100 股做半仓减没有意义**，趁仍浮盈全部离场。

它点名自己的买入论点并宣告证伪，引用的正是本计划合成的概念资金流；
日志里的演进是 `01-13 trim → 01-14 trim → Trim not actionable
(0 of 100股) — holding → 01-15 sell`，即先想减仓、发现手数减不动、才改为
全部离场。

### Learn：观察是可证伪的，而且敢自打脸

机械出场那轮写下的最后一条观察：

> 本窗口已平仓 7 笔中，T-1 涨幅高于中位数（+5.60%）的 3 笔中位收益
> -4.21%，低于或等于的 4 笔中位收益 -5.03%——逐笔看 4 笔支持、3 笔反对
> （n=7，远低于 n≥50，仅为观察）

`evidence_bundle` 带 `matching_rule` 与 `search_scope`，7 笔逐一标注
`supports true|false`，并且 `proposed_behavior_delta` 直接写明
**「该观察的对比方向与命题相反」**。

### Evolve：卡在人工审批，这是设计

全部停在 `observation`：n 远低于 50，`advance_candidate` 管线不调用。
生产库上的 v2 复盘把这件事说得更直白——`causal_trace_rate 0.0%`、
`decision_change_rate 0.0%`，即**它在记录自己还没有因为学习而改变行为**。
