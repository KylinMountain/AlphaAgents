...
def _interpret_signal(
    ...
    # 场景 R1: 严格 BC 信号（顶部派发，不进场）
    if _is_strict_BC(candidate, prior_markup_weeks):
        return ("strict_BC_top_warning", -80 + rs_bonus)

    # 场景 R2: markup 末段 (pos > 0.85) — 追高风险
    if weekly_phase == "markup" and pos_in_range > 0.85:
        return ("markup_late_avoid", -30 + rs_bonus)

    # 场景 R3: 月线 markup 末段 (月线 trend>+50% 且 pos > 0.85)
    if monthly_phase == "markup" and pos_in_range > 0.85:
        return ("monthly_top_avoid", -50 + rs_bonus)
