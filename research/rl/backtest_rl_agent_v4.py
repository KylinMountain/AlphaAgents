"""Backtest RL v4 agent on cyb 2026-01 → 2026-04."""
...
from rl_trading_env_v4 import SingleStockTradingEnvV3, SEQ_LEN
from train_rl_agent_v4 import CNNFeatureExtractorV3


def main():
    p = argparse.ArgumentParser()
    p.add_argument('--model', default='/Users/evilkylin/Projects/AlphaAgents/data/rl_trading_agent_v4.zip')
    ...
    if bh_returns:
        ...
        print(f'RL v4 avg per stock:  {avg:+.2f}%')
        print(f'Alpha:                {avg - bh:+.2f}pp')

    with open('/tmp/rl_v4_trades.jsonl', 'w') as g:
        for t in all_trades:
            g.write(json.dumps(t, ensure_ascii=False) + '\n')
    print(f'\nTrades log: /tmp/rl_v4_trades.jsonl')
