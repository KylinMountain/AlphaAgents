"""单股 RL trading environment v4.

升级 vs v3 (针对"教不会逃顶"的反馈):
1. SELL 时加入"逃顶 bonus": peak_dd<3 → +5; peak_dd>5 → -peak_dd
2. Terminal reward 改为连续: +alpha (1:1) + (total<0 ? total : 0) - dd_penalty
   不再分桶 — 50% 收益 ≠ 20% 收益
3. Hold 期 dd>5% 改用 ×0.5 系数(原 ×1 太重,把 hold 阶段奖励压扁)
"""
import numpy as np
    ...
    def step(self, action):
        ...
        if action == 1:  # BUY
            if self.pos == 0:
                ...
                reward -= 0.5  # 10x stronger (v3 had 78% INVALID spam)
            ...
            if self.pos == 1:
                ...
                # 逃顶激励: peak_dd 越小奖励越多
                if self.peak_dd < 3:
                    timing = 5.0  # 卖在峰值附近, 大奖励
                elif self.peak_dd > 5:
                    timing = -self.peak_dd  # 已经吃了回撤再卖, 罚 dd 大小
                else:
                    timing = 0.0
                reward += realized + timing - self.tc * 100
                ...
                info['timing_bonus'] = timing
            else:
                reward -= 0.5  # 10x stronger
            ...
            if self.pos == 1:
                ...
                if cur_dd > 5:
                    reward -= cur_dd * 0.5  # 软化 hold 阶段 dd 惩罚, 让 SELL 决策有空间
        ...
        if terminated:
            ...
            # Continuous terminal reward — 50% 收益 vs 20% 不再相同
            reward += alpha  # 1:1 with alpha (e.g. alpha=30 → +30, alpha=-15 → -15)
            if agent_total < 0:
                reward += agent_total  # 亏损额外加倍惩罚: total=-10 → 已经 alpha + 再 -10
            if self.worst_episode_dd > 10:
                reward -= self.worst_episode_dd * 0.5  # 总回撤过大额外罚
