    ...
    for code, entries in entries_by_code.items():
        ...
        for entry in entries:
            ...
            if df_full is not None:
                ...
                if len(df_slice) >= 10:
                    phase_context = _phase_guard_context_from_df(df_slice, as_of=entry["date"])
            ...
            try:
                ...
                # v5/v5.1/v5.2 vph fields — propagate so downstream analysis sees current validator output
                res["vp_harmony_score"] = verdict_data.get("vp_harmony_score", "neutral")
                res["vph_conflict"] = verdict_data.get("vph_conflict", {"exists": False})
                res["vph_conflict_failures"] = verdict_data.get("vph_conflict_failures", [])
                res["vph_conflict_validated"] = verdict_data.get("vph_conflict_validated", False)
