...
Run one preregistered validation window across the three isolated arms (B, C, D):
...
def _registered_capabilities(path: Path, manifest: dict) -> tuple[dict, str]:
    ...
    if errors:
        raise sector_experiment.SectorExperimentError(
            "formal B/C/D capability gate failed: " + "; ".join(errors))
...
def _compare(args) -> int:
    manifest = json.loads(args.manifest.read_text(encoding="utf-8"))
    arm_dirs = {
        arm: args.arms_dir / arm for arm in ("B", "C", "D")
    }
    report = sector_experiment_compare.compare(
